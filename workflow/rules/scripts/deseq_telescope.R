...
for (contrast in contrasts) {
    ...
    plotSave(paste(outputdir, tecounttype, contrast, "deplot.png", sep = "/"), p, 8, 8)
    ...
    write.csv(as.data.frame(resOrdered), file = paste(outputdir, tecounttype, contrast, "results.csv", sep = "/"))
    write.csv(as.data.frame(resSig), file = paste(outputdir, tecounttype, contrast, "resultsSig.csv", sep = "/"))
    write.csv(as.data.frame(counttablesizenormed), file = paste(outputdir, tecounttype, contrast, "counttablesizenormed.csv", sep = "/"))
    write.csv(as.data.frame(assay(vst_assay)), file = paste(outputdir, tecounttype, contrast, "rlogcounts.csv", sep = "/"))
    ...
    topSig <- resOrdered %>%
        head(n = 40) %>%
        rownames()
    topVarGenes <- head(order(rowVars(assay(vst_assay)[, rownames(df)]), decreasing = TRUE), 500)
    topVarGenes40 <- head(order(rowVars(assay(vst_assay)[, rownames(df)]), decreasing = TRUE), 40)
    ...
    for (name in names(interestingsets)) {
        for (scale in c("none", "row")) {
            for (binary in c(TRUE, FALSE)) {
                ...
                dirname <- file.path(outputdir, tecounttype, contrast)
                dir.create(dirname, recursive = TRUE)
                filename <- paste0(name, blabel, scale, ".png")
                path <- file.path(dirname, filename)
                png(path, width = 10, height = 8, units = "in", res = 300)
                pheatmap(assay(vst_assay)[set, rownames(df)], annotation_col = df, scale = scale, show_rownames = binary, main = name)
                dev.off()
            }
        }
    }
...
topVarGenes <- head(order(rowVars(assay(vst_assay)), decreasing = TRUE), 500)
topVarGenes40 <- head(order(rowVars(assay(vst_assay)), decreasing = TRUE), 40)
...
for (name in names(interestingsets)) {
    for (scale in c("none", "row")) {
        for (binary in c(TRUE, FALSE)) {
            ...
            set <- interestingsets[[name]]
            dirname <- file.path(outputdir, tecounttype, "plots")
            dir.create(dirname, recursive = TRUE)
            filename <- paste0(name, blabel, scale, ".png")
            path <- file.path(dirname, filename)
            png(path, width = 10, height = 8, units = "in", res = 300)
            pheatmap(assay(vst_assay)[set, ], annotation_col = df, scale = scale, show_rownames = binary, main = name)
            dev.off()
        }
    }
}
...
png(paste(outputdir, tecounttype, "plots", "screeplot.png", sep = "/"), width = 5, height = 6, units = "in", res = 300)
print(screep)
dev.off()
...
png(paste(outputdir, tecounttype, "plots", "loadings.png", sep = "/"), width = 6, height = 6, units = "in", res = 300)
print(loadingsp)
dev.off()
...
png(paste(outputdir, tecounttype, "plots", "pcaplot.png", sep = "/"), width = 7, height = 6, units = "in", res = 300)
print(pcap)
dev.off()
...
png(paste(outputdir, tecounttype, "plots", "pcaplotPC34.png", sep = "/"), width = 7, height = 6, units = "in", res = 300)
print(pcap)
dev.off()
...
png(paste(outputdir, tecounttype, "plots", "PCAgrid.png", sep = "/"), width = 16, height = 6, units = "in", res = 300)
print(p)
dev.off()
...
png(paste(outputdir, tecounttype, "plots", "heatmapplot.png", sep = "/"), width = 10, height = 8, units = "in", res = 300)
