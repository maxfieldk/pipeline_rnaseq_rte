...
# analysis parameters
{
    ...
    tryCatch(
        ...
        error = function(e) {
            ...
            assign("inputs", list(
                normcounttable = paste0("results/agg/deseq2/featurecounts_genes/", conf$contrasts, "/counttablesizenormed.csv")
            ), env = globalenv())
...
for (contrast in params[["contrasts"]]) {
    ...
    resdf <- read_csv(paste(params[["inputdir"]], contrast, "results.csv", sep = "/")) %>% rename(geneid = "...1")
    cts <- read_csv(inputs[["normcounttable"]]) %>% rename(geneid = "...1")
    res <- left_join(cts, resdf, by = "geneid") %>% arrange(-stat)

    ordered_by_stat <- setNames(res$stat, res$geneid) %>% na.omit()
    resl2fc <- res %>% arrange(-log2FoldChange)
    ordered_by_l2fc <- setNames(resl2fc$log2FoldChange, resl2fc$geneid) %>% na.omit()
...
for (contrast in params[["contrasts"]]) {
    ...
    for (collection in names(gse_results[[contrast]])) {
        ...
        for (geneset in names(core_enrichments)) {
            ...
            heatmapprep <- res %>% filter(geneid %in% genestoplot)
            m <- as.matrix(heatmapprep %>% dplyr::select(sample_vec))
            rownames(m) <- heatmapprep %$% geneid
            scaledm <- t(scale(t(m))) %>% na.omit()

            pvals <- heatmapprep %>%
                filter(geneid %in% rownames(scaledm)) %>%
                pull(padj)
...
for (geneset in names(params[["genesets_for_heatmaps"]])) {
    ...
    heatmapprep <- cts %>% filter(geneid %in% genestoplot)
    m <- as.matrix(heatmapprep %>% dplyr::select(sample_table$sample_name))
    rownames(m) <- heatmapprep %$% geneid
    scaledm <- t(scale(t(m))) %>% na.omit()

    topAnn <- HeatmapAnnotation(Condition = sample_table$condition, col = list(Condition = unlist(conf$condition_colors[sample_table$condition])))
    hm <- scaledm %>%
        Heatmap(
            name = "Scaled Normalized Counts",
            cluster_rows = FALSE,
